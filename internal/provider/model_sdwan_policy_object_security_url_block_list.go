...
func (data PolicyObjectSecurityURLBlockList) toBody(ctx context.Context) string {
	...
	body, _ = sjson.Set(body, "data.type", "urlblocked")
	path := "data."
	if true {
...
func (data *PolicyObjectSecurityURLBlockList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectSecurityURLBlockListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Pattern.ValueString() != data.Entries[ni].Pattern.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
