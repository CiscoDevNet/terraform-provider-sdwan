...
func (data *PolicyObjectSLAClassList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectSLAClassListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Latency.ValueInt64() != data.Entries[ni].Latency.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Loss.ValueInt64() != data.Entries[ni].Loss.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Jitter.ValueInt64() != data.Entries[ni].Jitter.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.AppProbeClassListId.ValueString() != data.Entries[ni].AppProbeClassListId.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.FallbackBestTunnelCriteria.ValueString() != data.Entries[ni].FallbackBestTunnelCriteria.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.FallbackBestTunnelLossVariance.ValueInt64() != data.Entries[ni].FallbackBestTunnelLossVariance.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.FallbackBestTunnelLatencyVariance.ValueInt64() != data.Entries[ni].FallbackBestTunnelLatencyVariance.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.FallbackBestTunnelJitterVariance.ValueInt64() != data.Entries[ni].FallbackBestTunnelJitterVariance.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
