...
func TestAccDataSourceSdwanPolicyObjectApplicationListProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_application_list_profile_parcel.test", "entries.0.application", "3com-amp3"))
...
func testAccDataSourceSdwanPolicyObjectApplicationListProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_application_list_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_application_list_profile_parcel" "test" {
			id = sdwan_policy_object_application_list_profile_parcel.test.id
