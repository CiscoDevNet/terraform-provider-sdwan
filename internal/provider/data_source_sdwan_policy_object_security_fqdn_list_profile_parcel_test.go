...
func TestAccDataSourceSdwanPolicyObjectSecurityFQDNListProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_security_fqdn_list_profile_parcel.test", "entries.0.pattern", "cisco.com"))
...
func testAccDataSourceSdwanPolicyObjectSecurityFQDNListProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_security_fqdn_list_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_security_fqdn_list_profile_parcel" "test" {
			id = sdwan_policy_object_security_fqdn_list_profile_parcel.test.id
