...
func TestAccDataSourceSdwanServiceLANVPNProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_service_lan_vpn_feature.test", "service_routes.0.sse_instance", "1"))
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_service_lan_vpn_feature.test", "static_nat_subnets.0.source_ip_subnet", "1.2.3.4"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_service_lan_vpn_feature.test", "static_nat_subnets.0.translated_source_ip_subnet", "2.3.4.5"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_service_lan_vpn_feature.test", "static_nat_subnets.0.prefix_length", "6"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_service_lan_vpn_feature.test", "static_nat_subnets.0.static_nat_direction", "inside"))
...
func testAccDataSourceSdwanServiceLANVPNProfileParcelByNameConfig() string {
	config := `resource "sdwan_service_lan_vpn_feature" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	config += ` description = "Terraform integration test"` + "\n"
	config += `	feature_profile_id = sdwan_service_feature_profile.test.id` + "\n"
