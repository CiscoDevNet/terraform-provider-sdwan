...
func TestAccDataSourceSdwanPolicyObjectAppProbeClassProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_app_probe_class_profile_parcel.test", "entries.0.map.0.color", "3g"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_app_probe_class_profile_parcel.test", "entries.0.map.0.dscp", "45"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_app_probe_class_profile_parcel.test", "entries.0.forwarding_class", "classlist1"))
...
func testAccDataSourceSdwanPolicyObjectAppProbeClassProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_app_probe_class_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_app_probe_class_profile_parcel" "test" {
			id = sdwan_policy_object_app_probe_class_profile_parcel.test.id
