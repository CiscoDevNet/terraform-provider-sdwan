...
type ServiceRoutingOSPFv3IPv4Redistributes struct {
	...
	EnableNatDia         types.Bool   `tfsdk:"enable_nat_dia"`
	EnableNatDiaVariable types.String `tfsdk:"enable_nat_dia_variable"`
	RoutePolicyId        types.String `tfsdk:"route_policy_id"`
}
...
func (data ServiceRoutingOSPFv3IPv4) toBody(ctx context.Context) string {
	...
	for _, item := range data.Redistributes {
		...
		if !item.EnableNatDiaVariable.IsNull() {
			itemBody, _ = sjson.Set(itemBody, "natDia.optionType", "variable")
			itemBody, _ = sjson.Set(itemBody, "natDia.value", item.EnableNatDiaVariable.ValueString())
		} else if item.EnableNatDia.IsNull() {
			...
			itemBody, _ = sjson.Set(itemBody, "natDia.value", item.EnableNatDia.ValueBool())
		}
	...
	for _, item := range data.Areas {
		...
		for _, childItem := range item.Ranges {
			...
			if !childItem.AddressVariable.IsNull() {
				...
				itemChildBody, _ = sjson.Set(itemChildBody, "network.address.value", childItem.AddressVariable.ValueString())
			} else if !childItem.Address.IsNull() {
				itemChildBody, _ = sjson.Set(itemChildBody, "network.address.optionType", "global")
				itemChildBody, _ = sjson.Set(itemChildBody, "network.address.value", childItem.Address.ValueString())
			}

			if !childItem.MaskVariable.IsNull() {
				itemChildBody, _ = sjson.Set(itemChildBody, "network.mask.optionType", "variable")
				itemChildBody, _ = sjson.Set(itemChildBody, "network.mask.value", childItem.MaskVariable.ValueString())
			} else if !childItem.Mask.IsNull() {
				itemChildBody, _ = sjson.Set(itemChildBody, "network.mask.optionType", "global")
				itemChildBody, _ = sjson.Set(itemChildBody, "network.mask.value", childItem.Mask.ValueString())
			}
...
func (data *ServiceRoutingOSPFv3IPv4) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(path + "redistribute"); value.Exists() {
		...
		value.ForEach(func(k, v gjson.Result) bool {
			...
			item.EnableNatDia = types.BoolNull()
			item.EnableNatDiaVariable = types.StringNull()
			if t := v.Get("natDia.optionType"); t.Exists() {
				va := v.Get("natDia.value")
				if t.String() == "variable" {
					item.EnableNatDiaVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					item.EnableNatDia = types.BoolValue(va.Bool())
				}
			}
...
func (data *ServiceRoutingOSPFv3IPv4) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Redistributes {
		...
		data.Redistributes[i].EnableNatDia = types.BoolNull()
		data.Redistributes[i].EnableNatDiaVariable = types.StringNull()
		if t := r.Get("natDia.optionType"); t.Exists() {
			va := r.Get("natDia.value")
			if t.String() == "variable" {
				data.Redistributes[i].EnableNatDiaVariable = types.StringValue(va.String())
			} else if t.String() == "global" {
				data.Redistributes[i].EnableNatDia = types.BoolValue(va.Bool())
			}
		}
	...
	for i := range data.Areas {
		...
		for ci := range data.Areas[i].Ranges {
			...
			keyValues := [...]string{data.Areas[i].Ranges[ci].Address.ValueString()}
			keyValuesVariables := [...]string{data.Areas[i].Ranges[ci].AddressVariable.ValueString()}
			...
			data.Areas[i].Ranges[ci].Address = types.StringNull()
			data.Areas[i].Ranges[ci].AddressVariable = types.StringNull()
			if t := cr.Get("network.address.optionType"); t.Exists() {
				va := cr.Get("network.address.value")
				if t.String() == "variable" {
					data.Areas[i].Ranges[ci].AddressVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Areas[i].Ranges[ci].Address = types.StringValue(va.String())
				}
			}
			data.Areas[i].Ranges[ci].Mask = types.StringNull()
			data.Areas[i].Ranges[ci].MaskVariable = types.StringNull()
			if t := cr.Get("network.mask.optionType"); t.Exists() {
				va := cr.Get("network.mask.value")
				if t.String() == "variable" {
					data.Areas[i].Ranges[ci].MaskVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Areas[i].Ranges[ci].Mask = types.StringValue(va.String())
				}
			}
