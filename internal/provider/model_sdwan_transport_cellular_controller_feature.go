...
func (data *TransportCellularController) isNull(ctx context.Context, res gjson.Result) bool {
	if !data.FeatureProfileId.IsNull() {
		return false
	}
	if !data.CellularId.IsNull() {
		return false
	}
	if !data.CellularIdVariable.IsNull() {
		return false
	}
	if !data.PrimarySimSlot.IsNull() {
		return false
	}
	if !data.PrimarySimSlotVariable.IsNull() {
		return false
	}
	if !data.SimFailoverRetries.IsNull() {
		return false
	}
	if !data.SimFailoverRetriesVariable.IsNull() {
		return false
	...
	if !data.FirmwareAutoSim.IsNull() {
		return false
	}
	if !data.FirmwareAutoSimVariable.IsNull() {
		return false
