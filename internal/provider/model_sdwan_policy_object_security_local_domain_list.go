...
func (data *PolicyObjectSecurityLocalDomainList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectSecurityLocalDomainListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.LocalDomain.ValueString() != data.Entries[ni].LocalDomain.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
