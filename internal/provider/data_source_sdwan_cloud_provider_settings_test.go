...
func TestAccDataSourceSdwanCloudProviderSettings(t *testing.T) {
	if os.Getenv("SDWAN_2015") == "" {
		t.Skip("skipping test, set environment variable SDWAN_2015")
	}
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_cloud_provider_settings.test", "umbrella_org_id", "123456"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_cloud_provider_settings.test", "umbrella_auth_key_v2", "aaaaaa"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_cloud_provider_settings.test", "umbrella_sig_auth_key", "bbbbbb"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_cloud_provider_settings.test", "umbrella_dns_auth_key", "cccccc"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_cloud_provider_settings.test", "zscaler_organization", "z223456"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_cloud_provider_settings.test", "zscaler_partner_base_uri", "zscaler.net"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_cloud_provider_settings.test", "zscaler_partner_key", "secret012"))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceSdwanCloudProviderSettingsConfig(),
...
func testAccDataSourceSdwanCloudProviderSettingsConfig() string {
	config := ""
	...
	config += `
		data "sdwan_cloud_provider_settings" "test" {
			depends_on = [sdwan_cloud_provider_settings.test]
		}
	`
	return config
}
