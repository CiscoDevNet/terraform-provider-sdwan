...
type VPNInterfaceDSLPPPoE struct {
	...
	Vdsl                                 []VPNInterfaceDSLPPPoEVdsl          `tfsdk:"vdsl"`
	...
	Encapsulation                        []VPNInterfaceDSLPPPoEEncapsulation `tfsdk:"encapsulation"`
	...
	PortForward                          []VPNInterfaceDSLPPPoEPortForward   `tfsdk:"port_forward"`
	...
	AccessList                           []VPNInterfaceDSLPPPoEAccessList    `tfsdk:"access_list"`
	Policer                              []VPNInterfaceDSLPPPoEPolicer       `tfsdk:"policer"`
...
type VPNInterfaceDSLPPPoEVdsl struct {
...
type VPNInterfaceDSLPPPoEEncapsulation struct {
	Optional           types.Bool   `tfsdk:"optional"`
...
type VPNInterfaceDSLPPPoEPortForward struct {
	Optional                 types.Bool   `tfsdk:"optional"`
...
type VPNInterfaceDSLPPPoEAccessList struct {
	Optional        types.Bool   `tfsdk:"optional"`
...
type VPNInterfaceDSLPPPoEPolicer struct {
	Optional    types.Bool   `tfsdk:"optional"`
	Direction   types.String `tfsdk:"direction"`
...
func (data *VPNInterfaceDSLPPPoE) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(path + "controller.vdsl.vipValue"); len(value.Array()) > 0 {
		data.Vdsl = make([]VPNInterfaceDSLPPPoEVdsl, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLPPPoEVdsl{}
	...
	if value := res.Get(path + "tunnel-interface.encapsulation.vipValue"); len(value.Array()) > 0 {
		data.Encapsulation = make([]VPNInterfaceDSLPPPoEEncapsulation, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLPPPoEEncapsulation{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
	...
	if value := res.Get(path + "nat.port-forward.vipValue"); len(value.Array()) > 0 {
		data.PortForward = make([]VPNInterfaceDSLPPPoEPortForward, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLPPPoEPortForward{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
	...
	if value := res.Get(path + "access-list.vipValue"); len(value.Array()) > 0 {
		data.AccessList = make([]VPNInterfaceDSLPPPoEAccessList, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLPPPoEAccessList{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
	...
	if value := res.Get(path + "policer.vipValue"); len(value.Array()) > 0 {
		data.Policer = make([]VPNInterfaceDSLPPPoEPolicer, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLPPPoEPolicer{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
