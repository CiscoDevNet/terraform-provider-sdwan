...
func (data *TransportRoutingOSPFv3IPv6) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultRedistributes := make([]TransportRoutingOSPFv3IPv6Redistributes, 0, len(data.Redistributes))
	...
	if !fullRead {
		resultAreas := make([]TransportRoutingOSPFv3IPv6Areas, 0, len(data.Areas))
		...
		for _, oldItem := range oldAreas {
			for ni := range data.Areas {
				...
				if keyMatch {
					...
					{
						resultC := make([]TransportRoutingOSPFv3IPv6AreasInterfaces, 0, len(data.Areas[ni].Interfaces))
					...
					{
						resultC := make([]TransportRoutingOSPFv3IPv6AreasRanges, 0, len(data.Areas[ni].Ranges))
						...
						for _, oldCItem := range oldItem.Ranges {
							for nci := range data.Areas[ni].Ranges {
								...
								keyMatchC := true
								if keyMatchC && (oldCItem.PrefixVariable.ValueString() != "" || data.Areas[ni].Ranges[nci].PrefixVariable.ValueString() != "") {
									if oldCItem.PrefixVariable.ValueString() != data.Areas[ni].Ranges[nci].PrefixVariable.ValueString() {
										keyMatchC = false
									}
								} else if keyMatchC {
									if oldCItem.Prefix.ValueString() != data.Areas[ni].Ranges[nci].Prefix.ValueString() {
										keyMatchC = false
									}
								}
								if keyMatchC {
