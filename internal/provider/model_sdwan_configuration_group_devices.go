...
type ConfigurationGroupDevices struct {
	...
	ConfigurationGroupVersion types.Int64                        `tfsdk:"configuration_group_version"`
	Devices                   []ConfigurationGroupDevicesDevices `tfsdk:"devices"`
}

type ConfigurationGroupDevicesDevices struct {
	Id            types.String                                `tfsdk:"id"`
	TopologyLabel types.String                                `tfsdk:"topology_label"`
	Deploy        types.Bool                                  `tfsdk:"deploy"`
	Variables     []ConfigurationGroupDevicesDevicesVariables `tfsdk:"variables"`
}

type ConfigurationGroupDevicesDevicesVariables struct {
...
func (data ConfigurationGroupDevices) getPath() string {
	return "/v1/config-group/"
}

// End of section. //template:end getPath

func (data ConfigurationGroupDevices) toBodyConfigGroupDevices(ctx context.Context) string {
	body := ""
	if true {
...
func (data ConfigurationGroupDevices) toBodyConfigGroupDeviceVariables(ctx context.Context, varTypes map[string]string) string {
	body := ""
...
func (data *ConfigurationGroupDevices) fromBodyConfigGroupDevices(ctx context.Context, res gjson.Result) {
	original := *data
	// Only manage devices already tracked in this resource's state. Devices associated with the same
	// configuration group by other resources or state files are intentionally ignored, so this
	// resource never reports drift for (or disassociates) devices it does not own.
	...
	if value := res.Get("devices"); value.Exists() && len(value.Array()) > 0 {
		data.Devices = make([]ConfigurationGroupDevicesDevices, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			if id := v.Get("id").String(); !stateDeviceIds[id] {
				return true
			}
			item := ConfigurationGroupDevicesDevices{}
		...
		if len(data.Devices) > 0 {
			data.Devices = []ConfigurationGroupDevicesDevices{}
		}
	}
	...
	for i := range original.Devices {
		...
		for y := range data.Devices {
			...
			if found {
				...
				data.Devices = append([]ConfigurationGroupDevicesDevices{device}, data.Devices...)
			}
		}
	}
}

func (data *ConfigurationGroupDevices) fromBodyConfigGroupDeviceVariables(ctx context.Context, res gjson.Result, original *ConfigurationGroupDevices) {
			...
			for i := range data.Devices {
				if data.Devices[i].Id.ValueString() == deviceId {
					if cValue := v.Get("variables"); cValue.Exists() && len(cValue.Array()) > 0 {
						...
						data.Devices[i].Variables = make([]ConfigurationGroupDevicesDevicesVariables, 0)
						cValue.ForEach(func(ck, cv gjson.Result) bool {
							// skip optional variables
							if !cv.Get("value").Exists() {
								return true
							}
							cItem := ConfigurationGroupDevicesDevicesVariables{}
						...
						if len(data.Devices[i].Variables) > 0 {
							data.Devices[i].Variables = []ConfigurationGroupDevicesDevicesVariables{}
						}
					}
					break
				}
...
func (data *ConfigurationGroupDevices) updateTfAttributes(ctx context.Context, state *ConfigurationGroupDevices) {
	for i := range data.Devices {
...
func (data ConfigurationGroupDevices) hasConfigGroupDeviceVariables(ctx context.Context) bool {
...
func (data ConfigurationGroupDevices) getUpdatedDevices(ctx context.Context, state *ConfigurationGroupDevices) []string {
...
func (data *ConfigurationGroupDevices) processImport(ctx context.Context) {
	for i := range data.Devices {
