...
func (data *PolicyObjectStandardCommunityList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectStandardCommunityListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.StandardCommunity.ValueString() != data.Entries[ni].StandardCommunity.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
