...
func (data *PolicyObjectClassMap) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectClassMapEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Queue.ValueString() != data.Entries[ni].Queue.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
