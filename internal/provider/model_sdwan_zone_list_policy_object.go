...
func (data ZoneListPolicyObject) toBody(ctx context.Context) string {
	...
	if true {
		...
		for _, item := range data.Entries {
			itemBody := ""
			if !item.Vpn.IsNull() {
				itemBody, _ = sjson.Set(itemBody, "vpn", item.Vpn.ValueString())
			}
...
func (data *ZoneListPolicyObject) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get("entries"); value.Exists() && len(value.Array()) > 0 {
		data.Entries = make([]ZoneListPolicyObjectEntries, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := ZoneListPolicyObjectEntries{}
...
func (data *ZoneListPolicyObject) hasChanges(ctx context.Context, state *ZoneListPolicyObject) bool {
	hasChanges := false
	...
	if len(data.Entries) != len(state.Entries) {
		...
		for i := range data.Entries {
			if !data.Entries[i].Vpn.Equal(state.Entries[i].Vpn) {
				hasChanges = true
			}
			if !data.Entries[i].Interface.Equal(state.Entries[i].Interface) {
				hasChanges = true
			}
		}
	}
...
func (data *ZoneListPolicyObject) processImport(ctx context.Context) {
	data.Version = types.Int64Value(0)
	data.Type = types.StringValue("zone")
}
