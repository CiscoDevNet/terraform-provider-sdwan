...
func TestAccDataSourceSdwanServiceRoutingBGPProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_service_routing_bgp_feature.test", "ipv6_neighbors.0.address_families.0.restart_max_number_of_prefixes", "2000"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_service_routing_bgp_feature.test", "ipv6_neighbors.0.address_families.0.restart_threshold", "75"))
...
func testAccDataSourceSdwanServiceRoutingBGPProfileParcelByNameConfig() string {
	config := `resource "sdwan_service_routing_bgp_feature" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	config += ` description = "Terraform integration test"` + "\n"
	config += `	feature_profile_id = sdwan_transport_feature_profile.test.id` + "\n"
