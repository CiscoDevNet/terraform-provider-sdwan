...
func (data TransportWANVPNInterfaceGRE) toBody(ctx context.Context) string {
	...
	if !data.MultiplexingVariable.IsNull() {
		if true {
			body, _ = sjson.Set(body, path+"basic.multiplexing.optionType", "variable")
			body, _ = sjson.Set(body, path+"basic.multiplexing.value", data.MultiplexingVariable.ValueString())
		}
	} else if data.Multiplexing.IsNull() {
		if true {
			body, _ = sjson.Set(body, path+"basic.multiplexing.optionType", "default")
			body, _ = sjson.Set(body, path+"basic.multiplexing.value", false)
		}
	} else {
		if true {
			body, _ = sjson.Set(body, path+"basic.multiplexing.optionType", "global")
			body, _ = sjson.Set(body, path+"basic.multiplexing.value", data.Multiplexing.ValueBool())
		}
	}
...
func (data *TransportWANVPNInterfaceGRE) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if t := res.Get(path + "basic.multiplexing.optionType"); t.Exists() {
		va := res.Get(path + "basic.multiplexing.value")
		if t.String() == "variable" {
