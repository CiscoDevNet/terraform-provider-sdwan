...
func (data *PolicyObjectApplicationList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectApplicationListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Application.ValueString() != data.Entries[ni].Application.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.ApplicationFamily.ValueString() != data.Entries[ni].ApplicationFamily.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
