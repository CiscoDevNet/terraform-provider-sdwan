...
func (data *PolicyObjectTLOCList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectTLOCListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.TlocIp.ValueString() != data.Entries[ni].TlocIp.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Color.ValueString() != data.Entries[ni].Color.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Encapsulation.ValueString() != data.Entries[ni].Encapsulation.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Preference.ValueString() != data.Entries[ni].Preference.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
