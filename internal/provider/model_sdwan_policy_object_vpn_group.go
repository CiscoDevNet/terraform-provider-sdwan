...
func (data *PolicyObjectVPNGroup) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectVPNGroupEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Vpn.ValueInt64() != data.Entries[ni].Vpn.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
