...
func (data *PolicyObjectSecurityIdentityList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectSecurityIdentityListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.User.ValueString() != data.Entries[ni].User.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.UserGroup.ValueString() != data.Entries[ni].UserGroup.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
