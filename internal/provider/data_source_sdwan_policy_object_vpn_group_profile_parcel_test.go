...
func TestAccDataSourceSdwanPolicyObjectVPNGroupProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_vpn_group_profile_parcel.test", "entries.0.vpn", "100"))
...
func testAccDataSourceSdwanPolicyObjectVPNGroupProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_vpn_group_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_vpn_group_profile_parcel" "test" {
			id = sdwan_policy_object_vpn_group_profile_parcel.test.id
