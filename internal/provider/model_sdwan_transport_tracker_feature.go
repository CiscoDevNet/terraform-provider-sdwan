...
func (data TransportTracker) toBody(ctx context.Context) string {
	...
	if !data.IntervalVariable.IsNull() {
		if true && (data.EndpointTrackerType.ValueString() == "interface" || data.EndpointTrackerType.IsNull()) {
	...
	if !data.IcmpIntervalVariable.IsNull() {
		if true && data.EndpointTrackerType.ValueString() == "interface-icmp" {
...
func (data *TransportTracker) fromBody(ctx context.Context, res gjson.Result) {
	...
	if t := res.Get(path + "interval.optionType"); t.Exists() {
		...
		data.EndpointTrackerType = types.StringValue("interface")
	}
	...
	if t := res.Get(path + "icmpInterval.optionType"); t.Exists() {
		...
		data.EndpointTrackerType = types.StringValue("interface-icmp")
	}
