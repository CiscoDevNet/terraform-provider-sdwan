...
func (data *PolicyObjectSecurityProtocolList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectSecurityProtocolListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.ProtocolName.ValueString() != data.Entries[ni].ProtocolName.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
