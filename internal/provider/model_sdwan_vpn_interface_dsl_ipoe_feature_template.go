...
type VPNInterfaceDSLIPoE struct {
	...
	EnableDhcpVariable                   types.String                       `tfsdk:"enable_dhcp_variable"`
	DhcpDistance                         types.Int64                        `tfsdk:"dhcp_distance"`
	DhcpDistanceVariable                 types.String                       `tfsdk:"dhcp_distance_variable"`
	DhcpHelper                           types.Set                          `tfsdk:"dhcp_helper"`
	DhcpHelperVariable                   types.String                       `tfsdk:"dhcp_helper_variable"`
	...
	Vdsl                                 []VPNInterfaceDSLIPoEVdsl          `tfsdk:"vdsl"`
	...
	Encapsulation                        []VPNInterfaceDSLIPoEEncapsulation `tfsdk:"encapsulation"`
	...
	PortForward                          []VPNInterfaceDSLIPoEPortForward   `tfsdk:"port_forward"`
	...
	AccessList                           []VPNInterfaceDSLIPoEAccessList    `tfsdk:"access_list"`
	Policer                              []VPNInterfaceDSLIPoEPolicer       `tfsdk:"policer"`
...
type VPNInterfaceDSLIPoEVdsl struct {
...
type VPNInterfaceDSLIPoEEncapsulation struct {
	Optional           types.Bool   `tfsdk:"optional"`
...
type VPNInterfaceDSLIPoEPortForward struct {
	Optional                 types.Bool   `tfsdk:"optional"`
...
type VPNInterfaceDSLIPoEAccessList struct {
	Optional        types.Bool   `tfsdk:"optional"`
...
type VPNInterfaceDSLIPoEPolicer struct {
	Optional    types.Bool   `tfsdk:"optional"`
	Direction   types.String `tfsdk:"direction"`
...
func (data VPNInterfaceDSLIPoE) toBody(ctx context.Context) string {
	...
	if !data.EnableDhcpVariable.IsNull() {
		...
		body, _ = sjson.Set(body, path+"ip.dhcp-client."+"vipVariableName", data.EnableDhcpVariable.ValueString())
	} else if data.EnableDhcp.IsNull() {
		body, _ = sjson.Set(body, path+"ip.dhcp-client."+"vipObjectType", "object")
		body, _ = sjson.Set(body, path+"ip.dhcp-client."+"vipType", "ignore")
	} else {
		body, _ = sjson.Set(body, path+"ip.dhcp-client."+"vipObjectType", "object")
		body, _ = sjson.Set(body, path+"ip.dhcp-client."+"vipType", "constant")
		body, _ = sjson.Set(body, path+"ip.dhcp-client."+"vipValue", strconv.FormatBool(data.EnableDhcp.ValueBool()))
	}
	...
	if !data.DhcpHelperVariable.IsNull() {
		body, _ = sjson.Set(body, path+"dhcp-helper."+"vipObjectType", "list")
		body, _ = sjson.Set(body, path+"dhcp-helper."+"vipType", "variableName")
		body, _ = sjson.Set(body, path+"dhcp-helper."+"vipVariableName", data.DhcpHelperVariable.ValueString())
	} else if data.DhcpHelper.IsNull() {
		body, _ = sjson.Set(body, path+"dhcp-helper."+"vipObjectType", "list")
		...
		var values []string
		data.DhcpHelper.ElementsAs(ctx, &values, false)
...
func (data *VPNInterfaceDSLIPoE) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(path + "ip.dhcp-client.vipType"); value.Exists() {
		if value.String() == "variableName" {
			...
			data.EnableDhcpVariable = types.StringValue(v.String())

		} else if value.String() == "ignore" {
			data.EnableDhcp = types.BoolNull()
			data.EnableDhcpVariable = types.StringNull()
		} else if value.String() == "constant" {
			v := res.Get(path + "ip.dhcp-client.vipValue")
			data.EnableDhcp = types.BoolValue(v.Bool())
			data.EnableDhcpVariable = types.StringNull()
		}
	} else {
		data.EnableDhcp = types.BoolNull()
	...
	if value := res.Get(path + "dhcp-helper.vipType"); len(value.Array()) > 0 {
		if value.String() == "variableName" {
			data.DhcpHelper = types.SetNull(types.StringType)

			v := res.Get(path + "dhcp-helper.vipVariableName")
			data.DhcpHelperVariable = types.StringValue(v.String())

		} else if value.String() == "ignore" {
			data.DhcpHelper = types.SetNull(types.StringType)
			data.DhcpHelperVariable = types.StringNull()
		} else if value.String() == "constant" {
			v := res.Get(path + "dhcp-helper.vipValue")
			data.DhcpHelper = helpers.GetStringSet(v.Array())
			data.DhcpHelperVariable = types.StringNull()
		}
	} else {
		data.DhcpHelper = types.SetNull(types.StringType)
	...
	if value := res.Get(path + "controller.vdsl.vipValue"); len(value.Array()) > 0 {
		data.Vdsl = make([]VPNInterfaceDSLIPoEVdsl, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLIPoEVdsl{}
	...
	if value := res.Get(path + "tunnel-interface.encapsulation.vipValue"); len(value.Array()) > 0 {
		data.Encapsulation = make([]VPNInterfaceDSLIPoEEncapsulation, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLIPoEEncapsulation{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
	...
	if value := res.Get(path + "nat.port-forward.vipValue"); len(value.Array()) > 0 {
		data.PortForward = make([]VPNInterfaceDSLIPoEPortForward, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLIPoEPortForward{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
	...
	if value := res.Get(path + "access-list.vipValue"); len(value.Array()) > 0 {
		data.AccessList = make([]VPNInterfaceDSLIPoEAccessList, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLIPoEAccessList{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
	...
	if value := res.Get(path + "policer.vipValue"); len(value.Array()) > 0 {
		...
		value.ForEach(func(k, v gjson.Result) bool {
			item := VPNInterfaceDSLIPoEPolicer{}
			if cValue := v.Get("vipOptional"); cValue.Exists() {
				item.Optional = types.BoolValue(cValue.Bool())
			} else {
				item.Optional = types.BoolNull()
...
func (data *VPNInterfaceDSLIPoE) hasChanges(ctx context.Context, state *VPNInterfaceDSLIPoE) bool {
	...
	if !data.EnableDhcp.Equal(state.EnableDhcp) {
		hasChanges = true
	}
	if !data.DhcpDistance.Equal(state.DhcpDistance) {
		hasChanges = true
	}
	if !data.DhcpHelper.Equal(state.DhcpHelper) {
		hasChanges = true
	}
