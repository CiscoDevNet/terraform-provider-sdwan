...
func (data *PolicyObjectSecurityIPSSignature) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectSecurityIPSSignatureEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.GeneratorId.ValueString() != data.Entries[ni].GeneratorId.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.SignatureId.ValueString() != data.Entries[ni].SignatureId.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
