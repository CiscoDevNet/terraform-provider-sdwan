...
func (data *PolicyObjectSecurityGeolocationList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Country.ValueString() != data.Entries[ni].Country.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Continent.ValueString() != data.Entries[ni].Continent.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
