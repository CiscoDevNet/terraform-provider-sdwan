...
func (data *ApplicationPriorityTrafficPolicy) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Sequences {
		...
		for ci := range data.Sequences[i].Actions {
			keys := [...]string{"slaClass", "set", "redirectDns.field", "redirectDns.value", "lossCorrection.lossCorrectionType", "lossCorrection.lossCorrectFec", "count", "log", "cloudSaas", "cloudProbe", "natPool", "nat.useVpn", "nat.fallback", "nat.bypass", "sig", "fallbackToRouting"}
