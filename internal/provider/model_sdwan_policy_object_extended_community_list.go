...
func (data *PolicyObjectExtendedCommunityList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectExtendedCommunityListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.ExtendedCommunity.ValueString() != data.Entries[ni].ExtendedCommunity.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
