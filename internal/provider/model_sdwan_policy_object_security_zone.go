...
func (data *PolicyObjectSecurityZone) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectSecurityZoneEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Vpn.ValueString() != data.Entries[ni].Vpn.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Interface.ValueString() != data.Entries[ni].Interface.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
