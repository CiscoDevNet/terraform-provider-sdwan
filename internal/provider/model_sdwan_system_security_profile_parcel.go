...
type SystemSecurity struct {
	...
	Key                              []SystemSecurityKey       `tfsdk:"key"`
}
...
func (data *SystemSecurity) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get(path + "key"); value.Exists() {
		data.Key = make([]SystemSecurityKey, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := SystemSecurityKey{}
...
func (data *SystemSecurity) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Key {
		...
		data.Key[i].Id = types.Int64Null()

		if t := r.Get("id.optionType"); t.Exists() {
			va := r.Get("id.value")
			if t.String() == "global" {
				data.Key[i].Id = types.Int64Value(va.Int())
			}
		}
		data.Key[i].Name = types.StringNull()

		if t := r.Get("name.optionType"); t.Exists() {
			va := r.Get("name.value")
			if t.String() == "global" {
				data.Key[i].Name = types.StringValue(va.String())
			}
		}
		data.Key[i].SendId = types.Int64Null()
		data.Key[i].SendIdVariable = types.StringNull()
		if t := r.Get("sendId.optionType"); t.Exists() {
			va := r.Get("sendId.value")
			if t.String() == "variable" {
				data.Key[i].SendIdVariable = types.StringValue(va.String())
			} else if t.String() == "global" {
				data.Key[i].SendId = types.Int64Value(va.Int())
			}
		}
		data.Key[i].ReceiverId = types.Int64Null()
		data.Key[i].ReceiverIdVariable = types.StringNull()
		if t := r.Get("recvId.optionType"); t.Exists() {
			va := r.Get("recvId.value")
			if t.String() == "variable" {
				data.Key[i].ReceiverIdVariable = types.StringValue(va.String())
			} else if t.String() == "global" {
				data.Key[i].ReceiverId = types.Int64Value(va.Int())
			}
		}
		data.Key[i].IncludeTcpOptions = types.BoolNull()
		data.Key[i].IncludeTcpOptionsVariable = types.StringNull()
		if t := r.Get("includeTcpOptions.optionType"); t.Exists() {
			va := r.Get("includeTcpOptions.value")
			if t.String() == "variable" {
				data.Key[i].IncludeTcpOptionsVariable = types.StringValue(va.String())
			} else if t.String() == "global" {
				data.Key[i].IncludeTcpOptions = types.BoolValue(va.Bool())
			}
		}
		data.Key[i].AcceptAoMismatch = types.BoolNull()
		data.Key[i].AcceptAoMismatchVariable = types.StringNull()
		if t := r.Get("acceptAoMismatch.optionType"); t.Exists() {
			va := r.Get("acceptAoMismatch.value")
			if t.String() == "variable" {
				data.Key[i].AcceptAoMismatchVariable = types.StringValue(va.String())
			} else if t.String() == "global" {
				data.Key[i].AcceptAoMismatch = types.BoolValue(va.Bool())
			}
		}
		data.Key[i].CryptoAlgorithm = types.StringNull()
		...
		if t := r.Get("sendLifetime.local.optionType"); t.Exists() {
			...
			if t.String() == "variable" {
				data.Key[i].SendLifeTimeLocalVariable = types.StringValue(va.String())
			} else if t.String() == "global" {
				data.Key[i].SendLifeTimeLocal = types.BoolValue(va.Bool())
			}
		}
		data.Key[i].SendLifeTimeStartEpoch = types.Int64Null()

		if t := r.Get("sendLifetime.startEpoch.optionType"); t.Exists() {
			va := r.Get("sendLifetime.startEpoch.value")
			if t.String() == "global" {
				data.Key[i].SendLifeTimeStartEpoch = types.Int64Value(va.Int())
			}
		}
		...
		data.Key[i].AcceptLifeTimeLocalVariable = types.StringNull()
		if t := r.Get("acceptLifetime.local.optionType"); t.Exists() {
			va := r.Get("acceptLifetime.local.value")
			if t.String() == "variable" {
				data.Key[i].AcceptLifeTimeLocalVariable = types.StringValue(va.String())
			} else if t.String() == "global" {
				data.Key[i].AcceptLifeTimeLocal = types.BoolValue(va.Bool())
			}
		}
		data.Key[i].AcceptLifeTimeStartEpoch = types.Int64Null()

		if t := r.Get("acceptLifetime.startEpoch.optionType"); t.Exists() {
			va := r.Get("acceptLifetime.startEpoch.value")
			if t.String() == "global" {
				data.Key[i].AcceptLifeTimeStartEpoch = types.Int64Value(va.Int())
			}
		}
