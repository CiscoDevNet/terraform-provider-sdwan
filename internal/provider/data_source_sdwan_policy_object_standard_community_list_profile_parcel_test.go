...
func TestAccDataSourceSdwanPolicyObjectStandardCommunityListProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_standard_community_list_profile_parcel.test", "entries.0.standard_community", "1000:10000"))
...
func testAccDataSourceSdwanPolicyObjectStandardCommunityListProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_standard_community_list_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_standard_community_list_profile_parcel" "test" {
			id = sdwan_policy_object_standard_community_list_profile_parcel.test.id
