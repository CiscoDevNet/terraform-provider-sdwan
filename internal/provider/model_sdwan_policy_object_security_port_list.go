...
func (data *PolicyObjectSecurityPortList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Port.ValueString() != data.Entries[ni].Port.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
