...
func (data *ServiceWirelessLAN) isNull(ctx context.Context, res gjson.Result) bool {
	if !data.FeatureProfileId.IsNull() {
		return false
	}
	if !data.Enable24g.IsNull() {
		return false
	}
	if !data.Enable24gVariable.IsNull() {
		return false
	}
	if !data.Enable5g.IsNull() {
		return false
	}
	if !data.Enable5gVariable.IsNull() {
		return false
