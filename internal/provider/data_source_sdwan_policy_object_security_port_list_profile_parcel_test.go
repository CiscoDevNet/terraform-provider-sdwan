...
func TestAccDataSourceSdwanPolicyObjectSecurityPortListProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_security_port_list_profile_parcel.test", "entries.0.port", "100"))
...
func testAccDataSourceSdwanPolicyObjectSecurityPortListProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_security_port_list_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_security_port_list_profile_parcel" "test" {
			id = sdwan_policy_object_security_port_list_profile_parcel.test.id
