...
func (data *ServiceMulticast) isNull(ctx context.Context, res gjson.Result) bool {
	if !data.FeatureProfileId.IsNull() {
		return false
	}
	if !data.SptOnly.IsNull() {
		return false
	}
	if !data.SptOnlyVariable.IsNull() {
		return false
	}
	if !data.LocalReplicator.IsNull() {
		return false
	}
	if !data.LocalReplicatorVariable.IsNull() {
		return false
	...
	if len(data.PimInterfaces) > 0 {
		return false
	}
	if len(data.StaticRpAddresses) > 0 {
		return false
	}
	if !data.EnableAutoRp.IsNull() {
		return false
	}
	if !data.EnableAutoRpVariable.IsNull() {
		return false
	}
	if len(data.AutoRpAnnounces) > 0 {
		return false
	}
	if len(data.AutoRpDiscoveries) > 0 {
		return false
	}
	if len(data.PimBsrRpCandidates) > 0 {
		return false
	}
	if len(data.PimBsrCandidates) > 0 {
		return false
	}
	if len(data.MsdpGroups) > 0 {
		return false
	}
	if !data.MsdpOriginatorId.IsNull() {
		return false
	}
	if !data.MsdpOriginatorIdVariable.IsNull() {
		return false
	}
	if !data.MsdpConnectionRetryInterval.IsNull() {
		return false
	}
	if !data.MsdpConnectionRetryIntervalVariable.IsNull() {
		return false
