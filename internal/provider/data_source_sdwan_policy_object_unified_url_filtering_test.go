...
func TestAccDataSourceSdwanPolicyObjectUnifiedURLFilteringProfileParcel(t *testing.T) {
	if os.Getenv("SDWAN_2012") == "" {
		t.Skip("skipping test, set environment variable SDWAN_2012")
	}
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_unified_url_filtering.test", "web_categories_action", "block"))
...
const testAccDataSourceSdwanPolicyObjectUnifiedURLFilteringPrerequisitesProfileParcelConfig = `
...
func testAccDataSourceSdwanPolicyObjectUnifiedURLFilteringProfileParcelConfig() string {
	...
	config += `
		data "sdwan_policy_object_unified_url_filtering" "test" {
			id = sdwan_policy_object_unified_url_filtering.test.id
			feature_profile_id = sdwan_policy_object_feature_profile.test.id
		}
	`
	return config
