...
func TestAccDataSourceSdwanPolicyObjectSecurityIdentityListProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_security_identity_list_profile_parcel.test", "entries.0.user", "administrator"))
...
func testAccDataSourceSdwanPolicyObjectSecurityIdentityListProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_security_identity_list_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_security_identity_list_profile_parcel" "test" {
			id = sdwan_policy_object_security_identity_list_profile_parcel.test.id
