...
func (data *PolicyObjectIPv4PrefixList) fromBody(ctx context.Context, res gjson.Result, fullRead bool) {
	...
	if !fullRead {
		resultEntries := make([]PolicyObjectIPv4PrefixListEntries, 0, len(data.Entries))
		...
		for _, oldItem := range oldEntries {
			for ni := range data.Entries {
				...
				keyMatch := true
				if keyMatch {
					if oldItem.Ipv4Address.ValueString() != data.Entries[ni].Ipv4Address.ValueString() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Ipv4PrefixLength.ValueInt64() != data.Entries[ni].Ipv4PrefixLength.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Le.ValueInt64() != data.Entries[ni].Le.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
					if oldItem.Ge.ValueInt64() != data.Entries[ni].Ge.ValueInt64() {
						keyMatch = false
					}
				}
				if keyMatch {
