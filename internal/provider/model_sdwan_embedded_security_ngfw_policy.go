...
type EmbeddedSecurityNGFW struct {
	...
	DefaultActione   types.String                `tfsdk:"default_actione"`
...
type EmbeddedSecurityNGFWRules struct {
	...
	Matches     []EmbeddedSecurityNGFWRulesMatches `tfsdk:"matches"`
	Actions     []EmbeddedSecurityNGFWRulesActions `tfsdk:"actions"`
}

type EmbeddedSecurityNGFWRulesMatches struct {
...
func (data EmbeddedSecurityNGFW) toBody(ctx context.Context) string {
	...
	path := "data."
	if !data.DefaultActione.IsNull() {
		if true {
			body, _ = sjson.Set(body, path+"defaultActionType.optionType", "global")
			body, _ = sjson.Set(body, path+"defaultActionType.value", data.DefaultActione.ValueString())
		}
	}
	if true {
	...
	if value := res.Get(path + "sequences"); value.Exists() && len(value.Array()) > 0 {
		...
		value.ForEach(func(k, v gjson.Result) bool {
			...
			if cValue := v.Get("match.entries"); cValue.Exists() && len(cValue.Array()) > 0 {
				item.Matches = make([]EmbeddedSecurityNGFWRulesMatches, 0)
				cValue.ForEach(func(ck, cv gjson.Result) bool {
					cItem := EmbeddedSecurityNGFWRulesMatches{}
...
func (data *EmbeddedSecurityNGFW) updateFromBody(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Rules {
		...
		keyValues := [...]string{data.Rules[i].SequenceId.ValueString(), data.Rules[i].RuleName.ValueString(), data.Rules[i].BaseAction.ValueString(), data.Rules[i].RuleType.ValueString(), strconv.FormatBool(data.Rules[i].DisableRule.ValueBool())}
		...
		data.Rules[i].SequenceId = types.StringNull()

		if t := r.Get("sequenceId.optionType"); t.Exists() {
			va := r.Get("sequenceId.value")
			if t.String() == "global" {
				data.Rules[i].SequenceId = types.StringValue(va.String())
			}
		}
		...
		for ci := range data.Rules[i].Matches {
			...
			keyValues := [...]string{helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceDataPrefixListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationDataPrefixListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationFqdnListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceGeoLocationListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationGeoLocationListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourcePortListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationPortListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceScalableGroupTagListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationScalableGroupTagListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceIndentityListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].ProtocolNameListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].AppListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].FlatAppListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceSecurityGroupListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationSecurityGroupListIds).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceDataPrefixs).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationDataPrefixs).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationFqdns).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourcePorts).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationPorts).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceGeoLocations).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].DestinationGeoLocations).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceIdentityUsers).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].SourceIdentityUsergroups).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].Applications).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].ApplicationFamilies).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].Protocols).ValueString(), helpers.GetStringFromSet(data.Rules[i].Matches[ci].ProtocolNames).ValueString()}
			keyValuesVariables := [...]string{"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", data.Rules[i].Matches[ci].SourceDataPrefixsVariable.ValueString(), data.Rules[i].Matches[ci].DestinationDataPrefixsVariable.ValueString(), data.Rules[i].Matches[ci].DestinationFqdnsVariable.ValueString(), data.Rules[i].Matches[ci].SourcePortsVariable.ValueString(), data.Rules[i].Matches[ci].DestinationPortsVariable.ValueString(), data.Rules[i].Matches[ci].SourceGeoLocationsVariable.ValueString(), data.Rules[i].Matches[ci].DestinationGeoLocationsVariable.ValueString(), "", "", "", "", "", ""}
			...
			data.Rules[i].Matches[ci].SourceDataPrefixListIds = types.SetNull(types.StringType)

			if t := cr.Get("sourceDataPrefixList.refId.optionType"); t.Exists() {
				va := cr.Get("sourceDataPrefixList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceDataPrefixListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationDataPrefixListIds = types.SetNull(types.StringType)

			if t := cr.Get("destinationDataPrefixList.refId.optionType"); t.Exists() {
				va := cr.Get("destinationDataPrefixList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationDataPrefixListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationFqdnListIds = types.SetNull(types.StringType)

			if t := cr.Get("destinationFqdnList.refId.optionType"); t.Exists() {
				va := cr.Get("destinationFqdnList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationFqdnListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourceGeoLocationListIds = types.SetNull(types.StringType)

			if t := cr.Get("sourceGeoLocationList.refId.optionType"); t.Exists() {
				va := cr.Get("sourceGeoLocationList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceGeoLocationListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationGeoLocationListIds = types.SetNull(types.StringType)

			if t := cr.Get("destinationGeoLocationList.refId.optionType"); t.Exists() {
				va := cr.Get("destinationGeoLocationList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationGeoLocationListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourcePortListIds = types.SetNull(types.StringType)

			if t := cr.Get("sourcePortList.refId.optionType"); t.Exists() {
				va := cr.Get("sourcePortList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].SourcePortListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationPortListIds = types.SetNull(types.StringType)

			if t := cr.Get("destinationPortList.refId.optionType"); t.Exists() {
				va := cr.Get("destinationPortList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationPortListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourceScalableGroupTagListIds = types.SetNull(types.StringType)

			if t := cr.Get("sourceScalableGroupTagList.refId.optionType"); t.Exists() {
				va := cr.Get("sourceScalableGroupTagList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceScalableGroupTagListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationScalableGroupTagListIds = types.SetNull(types.StringType)
			...
			if t := cr.Get("destinationScalableGroupTagList.refId.optionType"); t.Exists() {
				...
				if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationScalableGroupTagListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourceIndentityListIds = types.SetNull(types.StringType)

			if t := cr.Get("sourceIdentityList.refId.optionType"); t.Exists() {
				va := cr.Get("sourceIdentityList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceIndentityListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].ProtocolNameListIds = types.SetNull(types.StringType)

			if t := cr.Get("protocolNameList.refId.optionType"); t.Exists() {
				va := cr.Get("protocolNameList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].ProtocolNameListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].AppListIds = types.SetNull(types.StringType)

			if t := cr.Get("appList.refId.optionType"); t.Exists() {
				va := cr.Get("appList.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].AppListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].FlatAppListIds = types.SetNull(types.StringType)

			if t := cr.Get("appListFlat.refId.optionType"); t.Exists() {
				va := cr.Get("appListFlat.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].FlatAppListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourceSecurityGroupListIds = types.SetNull(types.StringType)

			if t := cr.Get("sourceSecurityGroup.refId.optionType"); t.Exists() {
				va := cr.Get("sourceSecurityGroup.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceSecurityGroupListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationSecurityGroupListIds = types.SetNull(types.StringType)

			if t := cr.Get("destinationSecurityGroup.refId.optionType"); t.Exists() {
				va := cr.Get("destinationSecurityGroup.refId.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationSecurityGroupListIds = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourceDataPrefixs = types.SetNull(types.StringType)
			data.Rules[i].Matches[ci].SourceDataPrefixsVariable = types.StringNull()
			if t := cr.Get("sourceIp.ipv4Value.optionType"); t.Exists() {
				va := cr.Get("sourceIp.ipv4Value.value")
				if t.String() == "variable" {
					data.Rules[i].Matches[ci].SourceDataPrefixsVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceDataPrefixs = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationDataPrefixs = types.SetNull(types.StringType)
			data.Rules[i].Matches[ci].DestinationDataPrefixsVariable = types.StringNull()
			if t := cr.Get("destinationIp.ipv4Value.optionType"); t.Exists() {
				va := cr.Get("destinationIp.ipv4Value.value")
				if t.String() == "variable" {
					data.Rules[i].Matches[ci].DestinationDataPrefixsVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationDataPrefixs = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationFqdns = types.SetNull(types.StringType)
			data.Rules[i].Matches[ci].DestinationFqdnsVariable = types.StringNull()
			if t := cr.Get("destinationFqdn.fqdnValue.optionType"); t.Exists() {
				va := cr.Get("destinationFqdn.fqdnValue.value")
				if t.String() == "variable" {
					data.Rules[i].Matches[ci].DestinationFqdnsVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationFqdns = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourcePorts = types.SetNull(types.StringType)
			data.Rules[i].Matches[ci].SourcePortsVariable = types.StringNull()
			if t := cr.Get("sourcePort.portValue.optionType"); t.Exists() {
				va := cr.Get("sourcePort.portValue.value")
				if t.String() == "variable" {
					data.Rules[i].Matches[ci].SourcePortsVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Rules[i].Matches[ci].SourcePorts = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationPorts = types.SetNull(types.StringType)
			data.Rules[i].Matches[ci].DestinationPortsVariable = types.StringNull()
			if t := cr.Get("destinationPort.portValue.optionType"); t.Exists() {
				va := cr.Get("destinationPort.portValue.value")
				if t.String() == "variable" {
					data.Rules[i].Matches[ci].DestinationPortsVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationPorts = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourceGeoLocations = types.SetNull(types.StringType)
			data.Rules[i].Matches[ci].SourceGeoLocationsVariable = types.StringNull()
			if t := cr.Get("sourceGeoLocation.optionType"); t.Exists() {
				va := cr.Get("sourceGeoLocation.value")
				if t.String() == "variable" {
					data.Rules[i].Matches[ci].SourceGeoLocationsVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceGeoLocations = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].DestinationGeoLocations = types.SetNull(types.StringType)
			data.Rules[i].Matches[ci].DestinationGeoLocationsVariable = types.StringNull()
			if t := cr.Get("destinationGeoLocation.optionType"); t.Exists() {
				va := cr.Get("destinationGeoLocation.value")
				if t.String() == "variable" {
					data.Rules[i].Matches[ci].DestinationGeoLocationsVariable = types.StringValue(va.String())
				} else if t.String() == "global" {
					data.Rules[i].Matches[ci].DestinationGeoLocations = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourceIdentityUsers = types.SetNull(types.StringType)

			if t := cr.Get("sourceIdentityUser.optionType"); t.Exists() {
				va := cr.Get("sourceIdentityUser.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceIdentityUsers = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].SourceIdentityUsergroups = types.SetNull(types.StringType)

			if t := cr.Get("sourceIdentityUsergroup.optionType"); t.Exists() {
				va := cr.Get("sourceIdentityUsergroup.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].SourceIdentityUsergroups = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].Applications = types.SetNull(types.StringType)

			if t := cr.Get("app.optionType"); t.Exists() {
				va := cr.Get("app.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].Applications = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].ApplicationFamilies = types.SetNull(types.StringType)

			if t := cr.Get("appFamily.optionType"); t.Exists() {
				va := cr.Get("appFamily.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].ApplicationFamilies = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].Protocols = types.SetNull(types.StringType)

			if t := cr.Get("protocol.optionType"); t.Exists() {
				va := cr.Get("protocol.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].Protocols = helpers.GetStringSet(va.Array())
				}
			}
			data.Rules[i].Matches[ci].ProtocolNames = types.SetNull(types.StringType)

			if t := cr.Get("protocolName.optionType"); t.Exists() {
				va := cr.Get("protocolName.value")
				if t.String() == "global" {
					data.Rules[i].Matches[ci].ProtocolNames = helpers.GetStringSet(va.Array())
				}
			}
		}
