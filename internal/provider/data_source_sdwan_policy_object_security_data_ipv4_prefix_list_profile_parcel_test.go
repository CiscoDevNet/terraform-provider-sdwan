...
func TestAccDataSourceSdwanPolicyObjectSecurityDataIPv4PrefixListProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_security_data_ipv4_prefix_list_profile_parcel.test", "entries.0.ip_prefix", "10.0.0.0/12"))
...
func testAccDataSourceSdwanPolicyObjectSecurityDataIPv4PrefixListProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_security_data_ipv4_prefix_list_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_security_data_ipv4_prefix_list_profile_parcel" "test" {
			id = sdwan_policy_object_security_data_ipv4_prefix_list_profile_parcel.test.id
