...
func TestAccDataSourceSdwanPolicyObjectASPathListProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_as_path_list_profile_parcel.test", "as_path_list_id", "1"))
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_as_path_list_profile_parcel.test", "entries.0.as_path_list", "110"))
...
func testAccDataSourceSdwanPolicyObjectASPathListProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_as_path_list_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_as_path_list_profile_parcel" "test" {
			id = sdwan_policy_object_as_path_list_profile_parcel.test.id
