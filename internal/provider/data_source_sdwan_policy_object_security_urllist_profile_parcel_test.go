...
func TestAccDataSourceSdwanPolicyObjectSecurityURLlistProfileParcel(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.sdwan_policy_object_security_urllist_profile_parcel.test", "entries.0.pattern", "www.cisco.com"))
...
func testAccDataSourceSdwanPolicyObjectSecurityURLlistProfileParcelConfig() string {
	config := `resource "sdwan_policy_object_security_urllist_profile_parcel" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	...
	config += `
		data "sdwan_policy_object_security_urllist_profile_parcel" "test" {
			id = sdwan_policy_object_security_urllist_profile_parcel.test.id
