...
func TestAccDataSourceSdwan{{camelCase .Name}}ProfileParcel(t *testing.T) {
	...
	{{- $config := . }}
	{{- range  .Attributes}}
	{{- if and (not .WriteOnly) (not .Encrypted) (not .ExcludeTest) (not .DynamicDefault) (not .TfOnly) (not .Reference) (not .Value) (not .TestValue)}}
	{{- if isNestedListSet .}}
	{{- $list := .TfName }}
	{{- if len .TestTags}}
	if {{range $i, $e := .TestTags}}{{if $i}} || {{end}}os.Getenv("{{$e}}") != ""{{end}} {
	...
	{{- end}}
	{{- range  .Attributes}}
	{{- if and (not .WriteOnly) (not .Encrypted) (not .ExcludeTest) (not .DynamicDefault) (not .TfOnly) (not .Reference) (not .Value) (not .TestValue) (not (isListSet .))}}
	{{- if len .TestTags}}
	if {{range $i, $e := .TestTags}}{{if $i}} || {{end}}os.Getenv("{{$e}}") != ""{{end}} {
...
{{- if not .NoDataSourceNameQuery}}
func testAccDataSourceSdwan{{camelCase .Name}}ProfileParcelByNameConfig() string {
	config := `resource "sdwan_{{getProfileParcelName $config}}" "test" {` + "\n"
	config += ` name = "TF_TEST"` + "\n"
	config += ` description = "Terraform integration test"` + "\n"
	{{- range  .Attributes}}
