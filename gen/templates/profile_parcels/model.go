...
func (data {{camelCase .Name}}) toBody(ctx context.Context) string {
	...
	path := "data."
	{{- range .Attributes}}
	{{- if and (or (eq .Type "String") (eq .Type "Int64") (eq .Type "Float64") (eq .Type "Bool") (isListSet .)) (not .Reference)}}
	{{if .Variable}}
	...
	} else{{else if .ExcludeNull}}if !data.{{toGoName .TfName}}.IsNull(){{else}}if !data.{{toGoName .TfName}}.IsNull(){{end}} {
	...
	for _, item := range data.{{toGoName .TfName}} {
		itemBody := ""
		{{- range .Attributes}}
		{{- if or (eq .Type "String") (eq .Type "Int64") (eq .Type "Float64") (eq .Type "Bool") (isListSet .)}}
		{{if .Variable}}
		...
		} else{{else if .ExcludeNull}}if !item.{{toGoName .TfName}}.IsNull(){{else}}if !item.{{toGoName .TfName}}.IsNull(){{end}} {
		...
		for _, childItem := range item.{{toGoName .TfName}} {
			...
			} else{{else if .ExcludeNull}}if !childItem.{{toGoName .TfName}}.IsNull(){{else}}if !childItem.{{toGoName .TfName}}.IsNull(){{end}} {
			...
			for _, childChildItem := range childItem.{{toGoName .TfName}} {
				...
				} else{{else if .ExcludeNull}}if !childChildItem.{{toGoName .TfName}}.IsNull(){{else}}if !childChildItem.{{toGoName .TfName}}.IsNull(){{end}} {
...
func (data *{{camelCase .Name}}) fromBody(ctx context.Context, res gjson.Result) {
	...
	{{- if and (or (eq .Type "String") (eq .Type "Int64") (eq .Type "Float64") (eq .Type "Bool") (isListSet .)) (not .Reference) (not .WriteOnly)}}
