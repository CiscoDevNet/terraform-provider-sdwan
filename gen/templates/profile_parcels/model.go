	...
	for i := range data.{{toGoName .TfName}} {
		...
		if t := r.Get("{{range .DataPath}}{{.}}.{{end}}{{.ModelName}}.optionType"); t.Exists() {
			...
			{{if .Variable}}if t.String() == "variable" {
				...
				{{- else if and (not (isListSet .)) .IncludeEmptyValue}}
				if va.Exists() {
					data.{{$list}}[i].{{toGoName .TfName}} = types.{{.Type}}Value(va.{{getGjsonType .Type}}())
				}
				{{- else}}
		...
		for ci := range data.{{$list}}[i].{{toGoName .TfName}} {
			...
			if t := cr.Get("{{range .DataPath}}{{.}}.{{end}}{{.ModelName}}.optionType"); t.Exists() {
				...
				{{if .Variable}}if t.String() == "variable" {
					...
					{{- else if and (not (isListSet .)) .IncludeEmptyValue}}
					if va.Exists() {
						data.{{$list}}[i].{{$clist}}[ci].{{toGoName .TfName}} = types.{{.Type}}Value(va.{{getGjsonType .Type}}())
					}
					{{- else}}
			...
			for cci := range data.{{$list}}[i].{{$clist}}[ci].{{toGoName .TfName}} {
				...
				if t := ccr.Get("{{range .DataPath}}{{.}}.{{end}}{{.ModelName}}.optionType"); t.Exists() {
					...
					{{if .Variable}}if t.String() == "variable" {
						...
						{{- else if and (not (isListSet .)) .IncludeEmptyValue}}
						if va.Exists() {
							data.{{$list}}[i].{{$clist}}[ci].{{$cclist}}[cci].{{toGoName .TfName}} = types.{{.Type}}Value(va.{{getGjsonType .Type}}())
						}
						{{- else}}
